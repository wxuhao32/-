/**
 * Core minesweeper logic (pure-ish, UI-agnostic).
 *
 * Features:
...
export function placeMines(grid, mineCount, safeR, safeC) {
  const rows = grid.length;
  const cols = grid[0].length;
  ...
  // (fallback: only exclude the first cell)
...
export function openCell(grid, r, c) {
  const rows = grid.length;
  const cols = grid[0].length;
  const cell = grid[r][c];
...
export function toggleFlag(grid, r, c) {
  const cell = grid[r][c];
...
export function countOpenSafe(grid) {
  let opened = 0;
  let totalSafe = 0;
...
export function chordOpen(grid, r, c) {
  const rows = grid.length;
  const cols = grid[0].length;
  const cell = grid[r][c];
